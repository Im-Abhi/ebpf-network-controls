 ...
 *   else (no rule matched, or all matches PASS) -> default policy
 *   unparseable / non-IPv4 -> default policy
 *
 * The default policy is read from the `config` map (entry 0 =
 * DEFAULT_ALLOW or DEFAULT_DENY). A single matched DROP always wins, so
 * adding an explicit block remains safe even under a default-allow policy,
 * and a default-deny policy cannot be bypassed by a single PASS rule.
 *
